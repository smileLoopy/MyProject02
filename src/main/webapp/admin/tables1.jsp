	...
	<c:otherwise>
		<c:forEach items="${boardLists }" var="row" varStatus="loop">
			<tr>
				<td>${row.idx }</td>
				<td>
					<a href="http://localhost:8082/MariaMVCBoard/mynotice/view_notice.do?idx=${row.idx }">${row.title }</a>
				</td>
				<td>${row.title }</td>
				<td>${row.name }</td>
				<td>${row.viewcount }</td>
         	...
         	<a href="../myboard/download.do?ofile=${row.ofile }&sfile=${row.sfile}&idx=${row.idx}">
       		[Down]</a>
                        ...
                        <button type="button" onclick="location.href='../mynotice/write_notice.do'">글쓰기</button>
                    </div>
                </main>
